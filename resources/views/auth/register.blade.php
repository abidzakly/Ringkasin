<x-guest-layout>
    <form method="POST" action="{{ route('register') }} " class="w-full lg:w-1/2 lg:ml-12 flex flex-col px-10 py-4">
        @csrf
        ...
        <div class="flex flex-col items-center justify-end mt-4">
            ...
            <a data-modal-target="login-modal" data-modal-toggle="login-modal" type="button"
                class="text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 hover:cursor-pointer">
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 'success') {
                            $('#popupsuccess').slideDown(500);

                            setTimeout(function() {
                                $('#popupsuccess').slideUp();
